...
def graph_eight():
    ...
    df.groupby('solicitacao')['centrodecusto'].sum().plot(kind='pie', autopct='%1.0f%%', startangle=10)
...
def graph_nine():
    df = pd.read_csv('./aguiartransportes/flaskr/downloadfile/empresa30dias.csv')
    plt.figure(figsize=(10, 10))
    df.groupby('tipo')['centrodecusto'].sum().plot(kind='pie', autopct='%1.0f%%', startangle=10)
